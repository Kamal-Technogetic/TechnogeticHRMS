    ...
    Mutation: {
        async registerEmployee(parent, args) {
            try {
                ...
                const { valid, errors } = validateRegisterInput(name, email, password, contactNumber, jobTitle, department, dateOfJoining);
                if (!valid) {
        ...
        async updateEmpDetails(parent, args, context) {
            try {
                ...
                const { gender, religion, nationality, dateOfBirth, fatherName, emergencyNumber, relationWithEmergencyNumber, } = args.input;
                // const { valid, errors } = validateLoginInput(email, password);
